...
const initialProfiles = [
  {
    id: 1,
    name: "Kieu Minh Trang",
    school: "FPTU",
    location: "Hòa Lạc, Hà Nội",
    workplace: "Công ty 1 thành viên",
...
export const Guess = (data) => {
  ...
  const [profiles, setProfiles] = useState(
    data.friend
      .slice(0, 4)
      .map((profile) => ({ ...profile, showDetails: false }))
  );
  ...
  const handleAddMoreResults = () => {
    ...
    const newProfiles = data.friend
      .slice(0, newDisplayCount)
      .map((profile) => ({ ...profile, showDetails: false }));
  ...
  const handleReset = () => {
    const resetProfiles = initialProfiles
      .slice(0, 4)
      .map((profile) => ({ ...profile, showDetails: false }));
  ...
  const checkPackage = () => {
    clientToken
      .get(`users/check-package/${user.id}`)
      .then((res) => {
