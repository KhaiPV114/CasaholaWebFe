    ...
    <Content>
      <Row className={s.section}>
        <Col span={12} className={s["section-left"]}>
          <Title level={1} className={s.title}>
            TÌM BẠN Ở GHÉP SIÊU CHẤT LƯỢNG!
          </Title>
          <Paragraph className={s.description}>
            Không còn nỗi lo ở ghép với người không phù hợp, đã có CasaHola giúp
            bạn.
          </Paragraph>
          {!!user ? (
            <Button
              onClick={() => navigate("/roompreference")}
              className={s.button}
            >
              BẮT ĐẦU NGAY
            </Button>
          ...
          <div className={s.features}>
            <div className={s["feature-item"]}>
              <CheckCircleFilled
                style={{ color: "#19BD00", fontSize: "20px" }}
              />
              Uy tín, chất lượng
            </div>
            <div className={s["feature-item"]}>
              <ClockCircleOutlined
                style={{ color: "#059AFF", fontSize: "20px" }}
              />
      ...
      <Row className={s.featureSection}>
        <Col span={24} className={s["featureSection-info"]}>
          <Title level={1} className={s.title}>
            TÍNH NĂNG TÌM BẠN Ở GHÉP CỦA CASAHOLA
          </Title>
          <Paragraph className={s.description}>
      ...
      <Row className={s.topSearch}>
        <Col span={24}>
          <Title level={1} className={s.topTitle}>
            TOP BẠN TRỌ ĐANG TÌM KIẾM
          </Title>
      ...
      <div className={s.cta}>
        <Col>
          <h1 className={s.title}>ROOMATE TÌM KHÔNG RA, CÓ CASAHOLA!</h1>
        </Col>
        <Col>
          <button className={s.button}>TRẢI NGHIỆM NGAY</button>{" "}
        </Col>
        <Row>
          <Col span={6}>
            {" "}
