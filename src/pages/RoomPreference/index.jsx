...
import { Card, Typography, Row, Col, Button, message, Form, Checkbox } from 'antd';
...
const RoomPreferenceForm = () => {
  ...
  const options = {
    ...
    hygiene: ['Dọn dẹp thường xuyên', 'Phân chia lịch dọn', 'Chỉ cần không quá bừa bộn', 'Không quan trọng', 'Thuê người dọn định kỳ', 'Tự dọn của mình'],
    outsider: ['Hạn chế người lạ đến phòng', 'Đến vào khung giờ nhất định', 'Chỉ cần không quá bừa bộn', 'Không quan trọng', 'Không cho người ngoài vào'],
  ...
  const validateForm = () => {
    ...
    if (missingFields.length > 0) {
      message.error(`Vui lòng chọn: ${missingFields.join(', ')}`);
      return false;
    }
  ...
  const onFinish = () => {
    ...
    const requiredFields = Object.keys(options);
    const data = requiredFields.filter(field => preferences[field] == "");


    console.log("Data gửi đi:", data);

    clientToken.post("/criterias/find-room-match", data)
    .then((data) => {
      console.log(data)
    })
  ...
  return (
    <Card className="roommate-preference-form">
      <div style={{ margin: 15 }}>
        <Title level={2} className="title">NHU CẦU BẠN TRỌ</Title>
        <Paragraph className="description">Bạn mong muốn tìm bạn trọ đáp ứng những tiêu chí nào?</Paragraph>
      </div>
      ...
      <Form form={form} layout="vertical" name="roommate_preference_form" onFinish={onFinish}>
        ...
        <div className="action-buttons">
          ...
          <Button className="next-button" shape="round" size="large" htmlType="submit">
            TÌM KIẾM
          </Button>
        </div>
      </Form>
